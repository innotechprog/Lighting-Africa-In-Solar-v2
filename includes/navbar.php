<?php
$nav_links = [
    ['url' => 'index.php', 'label' => t('nav_home')],
    ['url' => 'about.php', 'label' => t('nav_about')],
    ['url' => 'products.php', 'label' => t('nav_products')],
    ['url' => 'services.php', 'label' => t('nav_services')],
    ['url' => 'how-solar-works.php', 'label' => t('nav_how_solar_works')],
    ['url' => 'contact.php', 'label' => t('nav_contact')],
];
...
<nav class="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border">
    <div class="container mx-auto flex items-center justify-between h-[74px] px-4">
        ...
        <div class="hidden lg:flex items-center gap-1">
            <?php foreach ($nav_links as $link): ?>
                <?php $page = str_replace('.php', '', $link['url']); $active = ($page === 'index' && $current === 'index') || ($page !== 'index' && $current === $page); ?>
                <a href="<?php echo htmlspecialchars($link['url']); ?>" class="px-3 py-2 rounded-md text-sm font-medium transition-colors <?php echo $active ? 'text-primary' : 'text-muted-foreground hover:text-foreground'; ?>"><?php echo htmlspecialchars($link['label']); ?></a>
            <?php endforeach; ?>
        </div>
    ...
    <div class="lg:hidden mobile-menu hidden bg-background border-b border-border overflow-hidden">
        <div class="px-4 py-4 flex flex-col gap-2">
            <?php foreach ($nav_links as $link): ?>
                <?php $page = str_replace('.php', '', $link['url']); $active = ($page === 'index' && $current === 'index') || ($page !== 'index' && $current === $page); ?>
                <a href="<?php echo htmlspecialchars($link['url']); ?>" class="px-3 py-2 rounded-md text-sm font-medium transition-colors <?php echo $active ? 'text-primary bg-secondary' : 'text-muted-foreground hover:text-foreground'; ?>"><?php echo htmlspecialchars($link['label']); ?></a>
            <?php endforeach; ?>
            <a href="quote.php" class="inline-flex items-center justify-center rounded-md text-sm font-semibold bg-gradient-solar text-primary-foreground h-10 px-4 py-2 mt-2 w-full">Get a Quote</a>
        </div>
    </div>
</nav>
<script>
